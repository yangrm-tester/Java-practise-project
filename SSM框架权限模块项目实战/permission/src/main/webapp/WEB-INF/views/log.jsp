...
<script type="text/javascript">
    ...
    $(function () {
        ...
        function loadLogList() {
            //获取页面的查询参数
            /**获得页面参数*/
            var pageSize = $("#pageSize").val(); //每页数量
            var pageNo = $("#logPage .pageNo").val() || 1; //页码数
            var logType = $("#search-type").val(); //搜索类型
            var beforeSeg = $("#search-before").val(); //操作前的值
            var afterSeg = $("#search-after").val(); //操作后的值
            var operator = $("#search-operator").val(); //操作者
            var fromTime = $("#search-from").val(); //开始时间
            var toTime = $("#search-to").val(); //结束时间
            var url = "${ctx}/sys/log/page.json";
            $.ajax({
                url: url,
                method: "POST",
                data: {
                ...
                success: function (result) {
                    if (result.code == "0") {
                        ...
                        showMessage("获取日志列表", result.msg, false);
                    }
